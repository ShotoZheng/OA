...
<%@ page import="com.shoto.oa.global.Contant" %>
                ...
                <div class="panel-body pn">
                    <table id="message-table" class="table admin-form theme-warning tc-checkbox-1">
                        ...
                        <tr class="message-unread">
                            ...
                            <td>
                                <%--TODO 发现Contant.CLAIMVOUCHER_CREATED没有值，但是在该JSP页面中是导入成功的。
                                    TODO <c:out value = "Contant.CLAIMVOUCHER_CREATED">输出没有值，但是
                                    TODO System.out.println(Contant.CLAIMVOUCHER_CREATED); 可以输出对应值。
                                    TODO 环境原因?
                                --%>
                               <%-- <c:if test="${cv.status==Contant.CLAIMVOUCHER_CREATED || cv.status==Contant.CLAIMVOUCHER_BACK}">
                                ...
                                <c:if test="${cv.status.equals('新创建') || cv.status.equals('已打回')}">
                                    <a href="/claim_voucher/to_update?id=${cv.id}">修改</a>
                                    <a href="/claim_voucher/submit?id=${cv.id}">提交</a>
                                </c:if>
                                <c:if test="${cv.status.equals('已提交') || cv.status.equals('待复审')}">
                                    <a href="/claim_voucher/to_check?id=${cv.id}">审核</a>
                                </c:if>
                                <c:if test="${cv.status.equals('已审核')}">
                                    <a href="/claim_voucher/to_check?id=${cv.id}">打款</a>
                                </c:if>
